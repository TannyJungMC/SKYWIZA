...
import tannyjung.skywiza_handcode.misc.MiscOutside;
...
public class Spawner2 {
    ...
    public static int quadtree_level = 4; // 32 >> X
    ...
    public static void start (FeaturePlaceContext <NoneFeatureConfiguration> context) {
        ...
        if (file.exists() == false && file.isDirectory() == false) {
            ...
            region_gen = 4;
            region_gen_bar = 0;
            SkywizaMod.LOGGER.info("Generating Region (" + region_posX + "/" + region_posZ + ")");
                    ...
                    for (int scanZ = 0; scanZ < 32; scanZ++) {
                        ...
                        if (Math.random() < ConfigMain.region_scan_percentage * 0.01) {
                            ...
                            if (world_gen.hasChunk(chunk_posX, chunk_posZ) == false || (world_gen.hasChunk(chunk_posX, chunk_posZ) == true && world_gen.getChunk(chunk_posX, chunk_posZ).getStatus().isOrAfter(ChunkStatus.FULL)) == false) {
                                ...
                                getTreeData(world_gen, world, chunk_generator, center_posX, center_posZ);
            ...
            region_gen = 0;
            SkywizaMod.LOGGER.info("Completed!");

        }

    }

    private static void getTreeData (WorldGenLevel world_gen, ServerLevel world, ChunkGenerator chunk_generator, int center_posX, int center_posZ) {
        ...
        if (file.exists() == true) {
                                    ...
                                    if (skip == false) {
                                        ...
                                        if (read_all.startsWith("world_gen = ") == true) {
...

                                            {
                                                ...
                                                if (Misc.testConfigBiome(biome_center, biome) == false) {
            ...
            if (skip == false) {
                ...
                readTreeFile(world_gen, world, chunk_generator, tree_data, center_posX, center_posY, center_posZ);
    ...
    private static void readTreeFile (WorldGenLevel world_gen, ServerLevel world, ChunkGenerator chunk_generator, String tree_data, int center_posX, int center_posY, int center_posZ) {
